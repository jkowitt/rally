...
import { rallyMedia, type MediaData } from "@/lib/rally-api";
...
export default function MediaLibraryPage() {
  ...
  const loadMedia = useCallback(async () => {
    ...
    const res = await rallyMedia.list(params);
    if (res.ok && res.data) setMediaItems(res.data.media);
    setLoading(false);
  ...
  const handleFileUpload = async (files: File[]) => {
    ...
    for (const file of files) {
      ...
      const res = await rallyMedia.create({
        name: file.name,
        type,
        url: previewUrl,
  ...
  const deleteSelected = async () => {
    const res = await rallyMedia.bulkDelete(selectedItems);
    if (res.ok) {
  ...
  const deleteMedia = async (id: string) => {
    const res = await rallyMedia.delete(id);
    if (res.ok) {
    ...
    return (
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
          <div style={{ opacity: 0.3 }}>Loading media library...</div>
        </div>
      </div>
    );
  }
    ...
    <div>
        ...
        <div style={{ display: "flex", gap: "0.75rem", alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ position: "relative" }}>
            ...
            <svg
              ...
              style={{ position: "absolute", left: "0.75rem", top: "50%", transform: "translateY(-50%)", width: "18px", height: "18px", color: "var(--admin-text-secondary)" }}
            >
          ...
          <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="admin-form-select" style={{ width: "140px" }}>
          ...
          <div style={{ display: "flex", borderRadius: "8px", overflow: "hidden", border: "1px solid var(--admin-border)" }}>
            <button
              ...
              style={{ padding: "0.5rem 0.75rem", background: viewMode === "grid" ? "var(--admin-accent)" : "white", color: viewMode === "grid" ? "white" : "var(--admin-text)", border: "none", cursor: "pointer" }}
            >
            ...
            <button
              ...
              style={{ padding: "0.5rem 0.75rem", background: viewMode === "list" ? "var(--admin-accent)" : "white", color: viewMode === "list" ? "white" : "var(--admin-text)", border: "none", cursor: "pointer" }}
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18">
      ...
      {filteredMedia.length === 0 && (
        <div className="admin-card" style={{ textAlign: "center", padding: "3rem" }}>
          ...
          <h3 style={{ margin: "0 0 0.5rem" }}>No media items</h3>
          ...
          {filteredMedia.map((item) => (
            ...
            >
              ...
              <div className="admin-media-preview">
                {item.type === "image" && (item.thumbnailUrl || item.url) ? (
                  <img src={item.thumbnailUrl || item.url} alt={item.name} />
                ) : item.type === "video" ? (
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="5,3 19,12 5,21" /></svg>
                ) : (
              ...
              <div className="admin-media-info">
                ...
                <div className="admin-media-meta">{formatFileSize(item.size)} &bull; {item.type}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="admin-card" style={{ padding: 0 }}>
          <table className="admin-table">
            <thead>
              <tr>
                <th style={{ width: "40px" }}>
                  <input type="checkbox" onChange={(e) => { if (e.target.checked) { setSelectedItems(filteredMedia.map((m) => m.id)); } else { setSelectedItems([]); } }} />
                </th>
                <th>Preview</th>
                <th>Name</th>
                <th>Type</th>
            ...
            <tbody>
              {filteredMedia.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>
                    <div style={{ width: "60px", height: "40px", background: "var(--admin-bg)", borderRadius: "4px", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                      {item.type === "image" && (item.thumbnailUrl || item.url) ? (
                        <img src={item.thumbnailUrl || item.url} alt={item.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      ) : (
                  ...
                  <td><span className={`admin-badge admin-badge-${item.type === "image" ? "success" : item.type === "video" ? "info" : "warning"}`}>{item.type}</span></td>
                  <td>{formatFileSize(item.size)}</td>
                  <td style={{ color: "var(--admin-text-secondary)" }}>{new Date(item.createdAt).toLocaleDateString()}</td>
                  <td>
                    <div style={{ display: "flex", gap: "0.5rem" }}>
                      <button onClick={() => copyUrl(item.url)} className="admin-btn admin-btn-secondary" style={{ padding: "0.25rem 0.5rem" }}>Copy URL</button>
                      <button onClick={() => setSelectedMedia(item)} className="admin-btn admin-btn-secondary" style={{ padding: "0.25rem 0.5rem" }}>View</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="admin-modal-overlay" onClick={() => setShowUploadModal(false)}>
          <div className="admin-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "500px" }}>
            ...
            <div className="admin-modal-body">
              ...
              >
                <input ref={fileInputRef} type="file" multiple accept="image/*,video/*,.pdf,.doc,.docx" onChange={handleFileSelect} style={{ display: "none" }} />
                <div className="admin-upload-icon">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4" /><polyline points="17,8 12,3 7,8" /><line x1="12" y1="3" x2="12" y2="15" />
                  </svg>
                </div>
                <div className="admin-upload-text">{uploading ? "Uploading..." : "Drag and drop files here, or click to browse"}</div>
                <div className="admin-upload-hint">Supports images, videos, and documents up to 50MB</div>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="admin-modal-body">
              <div style={{ background: "var(--admin-bg)", borderRadius: "8px", padding: "1rem", marginBottom: "1rem" }}>
                {selectedMedia.type === "image" ? (
                  <img src={selectedMedia.url} alt={selectedMedia.name} style={{ maxWidth: "100%", maxHeight: "300px", display: "block", margin: "0 auto" }} />
                ) : selectedMedia.type === "video" ? (
                  <video src={selectedMedia.url} controls style={{ maxWidth: "100%", maxHeight: "300px", display: "block", margin: "0 auto" }} />
                ) : (
              ...
              <div className="admin-form-group">
                ...
                <input type="text" className="admin-form-input" value={selectedMedia.tags.join(", ") || "No tags"} readOnly />
              </div>
              ...
              <div className="admin-form-group">
                ...
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  ...
                  <button onClick={() => copyUrl(selectedMedia.url)} className="admin-btn admin-btn-secondary">Copy</button>
                </div>
              </div>
            </div>
            <div className="admin-modal-footer">
              <button onClick={() => deleteMedia(selectedMedia.id)} className="admin-btn admin-btn-danger">Delete</button>
              <button onClick={() => { copyUrl(selectedMedia.url); setSelectedMedia(null); }} className="admin-btn admin-btn-primary">Copy URL &amp; Close</button>
            </div>
          </div>
        </div>
      )}
