...
/**
 * /auth/outlook/callback — customer-facing Outlook OAuth landing page.
 *
 * Microsoft redirects here after the user grants consent on the
 * Microsoft sign-in screen. We forward the authorization code to
 * the outlook-auth edge function, which exchanges it for tokens,
 * stores them encrypted in outlook_auth, and returns the
 * connected email address.
 *
 * On success, we redirect to /app/crm/inbox/connect so the user
 * sees their connection card with "Connected" status.
 */
export default function OutlookOAuthCallback() {
  const [params] = useSearchParams()
  const [state, setState] = useState({ phase: 'working', message: 'Finishing Outlook sign-in…' })
  ...
  useEffect(() => {
    ...
    supabase.functions.invoke('outlook-auth', { body: { action: 'exchange_code', code } })
